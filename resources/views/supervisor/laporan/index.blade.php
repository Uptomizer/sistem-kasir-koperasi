...
<div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 mb-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex gap-3">
            <a href="{{ route('supervisor.laporan.export_pdf') }}" target="_blank"
            ...
            <a href="{{ route('supervisor.laporan.export') }}"
               onclick="this.href = this.href.split('?')[0] + window.location.search;"
...
<div id="laporan-list" class="space-y-6">
    @include('supervisor.laporan.partials.list')
</div>
...
<script>
    ...
    function openDetailModal(id) {
        ...
        fetch(`{{ url('supervisor/transaksi') }}/${id}`)
            .then(res => res.json())
            .then(data => {
                let itemsHtml = '';
